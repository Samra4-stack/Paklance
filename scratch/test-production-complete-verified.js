...
(async () => {
  ...
  const clientUser = {
    id: 'prod_smoke_client',
    name: 'Karachi Tech Ventures',
    role: 'CLIENT',
    email: 'smoke_client@paklance.com'
  };

  // Set persistent auth in browser localStorage
  ...
  console.log(`=== STARTING FINAL VERIFIED PRODUCTION SMOKE TESTS ===\nTarget: ${PROD_URL}\n`);
  ...
  // Register all mock routes BEFORE page.goto
  await page.route('**/api/jobs?clientId=*', async route => {
    await route.fulfill({
      status: 200,
  ...
  record('11. Wallet Page loads correctly', walletView);
